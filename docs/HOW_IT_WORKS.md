...
9. **Redeploy (optional).** With *Settings → Git & GitHub → Redeploy after merge* enabled and the task's workflow opted in, a watcher checks the pull request and runs your redeploy command once it is merged (or right after delivery, if you chose that trigger). The command is owner configuration only: it never comes from the task text, from agent output or from a request body, and agents cannot change it. One deployment runs at a time, in its own process group with your timeout, and the full output lands in `runtime/<task>/redeploy.log`. The task page shows the status next to the pull request and offers *Redeploy now*, which runs the command whatever the merge state is.
...
When a task ends, Relay scores it (0 to 100, formula in `orchestrator/scorecard.py`) and keeps checking its pull request for two weeks: merged, closed unmerged, or fixed by a human after delivery. One short retrospective turn (the supervisor's agent at its lowest effort, on its cheap subagent model when one is set) reads a digest of the run and proposes a few one-sentence lessons. They wait in **Knowledge → Lessons** until you approve, edit or reject them; approved lessons for a repository, plus global ones, are added to the supervisor's and worker's kickoff prompts on later tasks (15 at most). Mission Control shows the 14-day success trend; **Knowledge → Learning** breaks results down by repository and team, and why tasks fail.
