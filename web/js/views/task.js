...
import { S, agentLabel, ROLE_LABEL, roleAgent, roleModel, roleEffort, statusOf, LIVE, msgStore, taskElapsed, navigate, bus } from "../state.js";
...
export function mountTask(main, id) {
  ...
  function renderHeader() {
    ...
    const team = ["supervisor", "worker", "reviewer"].filter((r) => roleAgent(t, r)).map((r) => {
      ...
      return `<span class="team-pill ${on ? "on" : ""}" title="${esc(ROLE_LABEL[r])}: ${esc(agentLabel(a))} · ${esc(roleModel(t, r) || "default model")}${roleEffort(t, r) ? ` · ${esc(roleEffort(t, r))} effort` : ""}"><span class="av xs ${esc(a)}"></span><span class="tpl-role">${esc(ROLE_LABEL[r])}</span><span class="tpl-agent">${esc(agentLabel(a))}</span>${on ? '<span class="live-dot sm"></span>' : ""}</span>`;
