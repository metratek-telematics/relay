  ...
  function draw() {
    ...
    body.innerHTML = `
      ...
      <div class="card"><div class="card-head"><h3>People</h3><span class="badge">${users.length}</span></div><div class="card-body org-table-wrap" style="padding:4px 8px">
        <table class="org-table"><thead><tr><th>Person</th><th>Role</th><th class="org-hide-sm">Groups</th><th>Last seen</th>${can("admin") ? '<th class="num org-hide-sm">Budget / month</th>' : ""}${owner ? "<th></th>" : ""}</tr></thead><tbody>
        ${users.map((u) => `<tr style="${u.disabled ? "opacity:.55" : ""}">
          <td><div class="org-person">${avatar(u, 32)}<div class="min0"><strong class="truncate">${esc(u.name || u.username)}${u.username === me ? ' <span class="badge outline">you</span>' : ""}${u.disabled ? ' <span class="badge red">disabled</span>' : ""}</strong><span class="muted truncate">${esc(u.email || u.username)}</span></div></div></td>
          ...
          <td>${u.last_seen ? esc(timeAgo(u.last_seen)) : '<span class="muted">never</span>'}</td>
          ...
          ${owner ? `<td class="num">${u.username !== me && !u.local ? `<button class="btn xs ghost" data-toggle="${esc(u.username)}" data-disabled="${u.disabled ? 1 : 0}">${u.disabled ? "Enable" : "Disable"}</button><button class="btn xs ghost" data-remove="${esc(u.username)}" title="Remove">${icon("trash")}</button>` : ""}</td>` : ""}
