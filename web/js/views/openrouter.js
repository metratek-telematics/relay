...
export function openRouterCard() {
  ...
  return `<div class="card" id="orCard"><div class="card-head"><h3>${icon("layers", "sm")} OpenRouter</h3><span class="badge ${p.configured ? "green" : "amber"}">${p.configured ? "ready" : "no key"}</span></div><div class="card-body hint stack">
    ...
    <div><span class="badge purple" title="Billed per token from a balance, not a subscription">pay as you go</span> OpenRouter spends credit per token instead of a plan, so there is no window to wait for — only credit to run out of. The automatic free pick rotates through free models, which cost nothing and have their own daily request cap. Credit left and what Relay spent are under Models &amp; usage; anything unread says “not known” there rather than 0.</div>
