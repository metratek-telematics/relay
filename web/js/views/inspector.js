...
import { S, agentLabel, agentInitial, ROLE_LABEL, roleAgent, statusOf, taskElapsed } from "../state.js";
import { roleNowHtml } from "../rolecontrol.js";
    ...
    body.innerHTML = `
      <div class="stack" style="gap:14px">
        ...
        <div class="card"><div class="card-head"><h3>Team</h3><span class="row" style="gap:6px">${(t.triage || t.triage_hint) ? `<span class="badge ${(t.triage || t.triage_hint).mode === "solo" ? "green" : "purple"}" title="${esc((t.triage || t.triage_hint).why || "")}">${(t.triage || t.triage_hint).mode === "solo" ? "Solo" : "Team"}${t.triage ? "" : " (expected)"}</span>` : ""}<span class="badge outline">${esc(wf.preset || "custom")}</span></span></div><div class="card-body stack">
          ${roles.map((role) => roleNowHtml(t, role)).join("")}
          <div class="kv" style="margin-top:6px"><dt>Verification</dt><dd>${esc(wf.verify_mode || "each_report")}${(t.verify_commands || []).length ? ` · ${t.verify_commands.length} command(s)` : ""}</dd><dt>Approval gate</dt><dd>${wf.approval_before_delivery ? "before delivery" : "off"}</dd><dt>Agent questions</dt><dd>${wf.allow_agent_questions === false ? "disabled" : "allowed"}</dd><dt>Review rounds</dt><dd>${wf.max_review_rounds || "not limited"}</dd><dt>Team mode</dt><dd>${esc(wf.team_mode || "auto")}${t.triage ? ` → ${esc(t.triage.mode)} (${esc(t.triage.level)})` : ""}</dd></div>
        </div></div>
        ${autopilotCard(t)}
        ...
        <div class="card"><div class="card-head"><h3>Signals</h3></div><div class="card-body stack">
          ...
          <div class="row between"><span>Independent review</span>${r ? `<span class="badge ${r.verdict === "PASS" ? "green" : "red"}">${esc(r.verdict)} · round ${r.round}</span>` : roleAgent(t, "reviewer") ? '<span class="muted">not run yet</span>' : '<span class="muted">no reviewer on this team</span>'}</div>
          <div class="row between"><span>Diff</span>${t.diffstat ? `<span class="diffstat">${t.diffstat.files} files <span class="a">+${t.diffstat.insertions}</span> <span class="d">−${t.diffstat.deletions}</span></span>` : `<span class="muted">${t.started_at ? "nothing changed yet" : "not known until the task starts"}</span>`}</div>
          <div class="row between"><span>Branch</span><span class="mono truncate" style="max-width:240px">${t.branch ? esc(t.branch) : '<span class="muted">created when the task starts</span>'}</span></div>
          <div class="row between"><span>Pull request</span>${t.pr_url ? `<a href="${esc(t.pr_url)}" target="_blank" rel="noopener">#${esc(t.pr_number || "")} ${icon("external", "sm")}</a>` : '<span class="muted">not opened yet</span>'}</div>
          <div class="row"><button type="button" class="btn xs" data-open-tab="delivery">${icon("package", "sm")}See the whole story, request to live</button></div>
        </div></div>
