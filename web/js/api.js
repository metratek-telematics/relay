...
export const api = {
  ...
  knowledgeDocs: () => get("/api/knowledge/docs"),
  knowledgeDoc: (path) => get(`/api/knowledge/doc?path=${encodeURIComponent(path)}`),
  knowledgeSearch: (q) => get(`/api/knowledge/search?q=${encodeURIComponent(q)}`),
  knowledgeSave: (body) => post("/api/knowledge/docs/save", body),
  knowledgeRefresh: (body = {}) => post("/api/knowledge/refresh", body),
